...
export default function StockDetails({ selectedStock, searchQuery, setHighPrice, setLowPrice, setOpenPrice,
        ...
        try {
            ...
            if (starFill === "bi bi-star-fill m-2") {
                ...
                const response = await httpCall({
                    http: `${process.env.REACT_APP_API_HOST}/watchlist/mongo/${searchQuery}`,
                    method: "POST",
                    body: { check: "remove", exchange: exchange }
                });
            }
            else {
                ...
                const response = await httpCall({
                    http: `${process.env.REACT_APP_API_HOST}/watchlist/mongo/${searchQuery}`,
                    method: "POST",
                    body: { check: "add", exchange: exchange }
                });
            }
    ...
    useEffect(() => {
        fetchMongoStock();
    }, [selectedStock, searchQuery, sellButtonState]);
    ...
    const fetchMongoStock = async () => {
        try {
            ...
            const hasTickerSymbol = data.stockHolding.some(item => item.tickerSymbol.toUpperCase() === tickerSymbol.toUpperCase());
            console.log(hasTickerSymbol)
            if (hasTickerSymbol) { setSellButtonState(true); }
            else { setSellButtonState(false); }
        } catch (error) {
            console.error('Error fetching chart data:', error);
            throw error;
        }
    ...
    const handleBuyButton = async () => {
        try {
            const response = await httpCall({
                http: `${process.env.REACT_APP_API_HOST}/search/mongo/${searchQuery.toUpperCase()}`,
                method: "POST",
                    ...
                    <div className="stock-detail-container">
                        <span className="d-flex flex-column justify-content-center align-items-center">
                            <div className="d-flex align-items-center" style={{ fontSize: 'xx-large', fontWeight: '400', height: '30px' }}>
                                {tickerSymbol}<i id="favIcon" className={starFill} style={{ color: starColor }} onClick={addToFav}></i>
                            </div>
                            <div className="" style={{ fontSize: 'x-large', fontWeight: '400' }}>{companyName}</div>
                            <div style={{ fontSize: 'small', alignItems: "center", justifyContent: "center", textAlign: "center" }}>{exchange}</div>
                            <span >
                                <Button variant="contained" style={{ backgroundColor: 'green', textTransform: 'none', marginTop: '10px' }} disableElevation onClick={openBuyModal}>
                                    Buy
                                </Button>
                                {sellButtonState && (
                                    <Button variant="contained" style={{ backgroundColor: 'red', textTransform: 'none', marginTop: '10px', marginLeft: '10px' }} disableElevation onClick={openSellModal}>
                                        Sell
                                    </Button>
                                )}
                                {/* Buy Modal */}
                                <Modal className="modal-dialog-centered modal-dialog-scrollable modal-dialog-centered-responsive" open={isBuyModalOpen} onClose={closeBuyModal}>
                                    <Box sx={style}>
                                        ...
                                        <div className="d-flex flex-column mx-4 my-2">
                                            ...
                                            <span>Money in Wallet: {walletBalance}</span>
                                            <div>Quantity:
                                                <TextField
                                                    className="mx-2"
                                                    type="number"
                                        ...
                                        <div style={{ borderTop: "1px solid #ccc", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                                            ...
                                            <Button className="mx-3 mt-2" variant="contained" style={{ backgroundColor: 'green', textTransform: 'none' }} disableElevation onClick={handleBuyButton}>
                                                Buy
                                            </Button>
                                        </div>
                                    </Box>
                                    ...
                                    <div className="modal" tabIndex="-1" style={{ display: 'block' }}>
                                        <div className="modal-content">
                                            <div className="modal-header">
                                                <h5 className="modal-title">Sell Stock</h5>
                                                <button type="button" className="btn-close" onClick={closeSellModal}></button>
                                            </div>
                                            <div className="modal-body">
                                                <p>Placeholder for Sell modal content.</p>
                                            </div>
                                            <div className="modal-footer">
                                                <button type="button" className="btn btn-secondary" onClick={closeSellModal}>Close</button>
                                            </div>
                                        </div>
                                    </div>
                                )}
